defmodule BaseAclEx.Infrastructure.Security.JWT.GuardianImpl do
  ...
  def generate_tokens(%User{} = user) do
    with {:ok, access_token, access_claims} <- encode_and_sign(user, %{}, 
           token_type: "access",
           ttl: {15, :minutes}),
         {:ok, refresh_token, refresh_claims} <- encode_and_sign(user, %{}, 
           token_type: "refresh",
           ttl: {7, :days}) do
  ...
  def validate_permissions(token, required_permissions) do
    with {:ok, claims} <- decode_and_verify(token),
      ...
      has_all_permissions = Enum.all?(required_permissions, fn perm ->
  ...
  defp maybe_rotate_refresh_token(old_token, user) do
    ...
    if should_rotate_token?(old_token) do
      {:ok, new_token, _claims} = encode_and_sign(user, %{}, 
        token_type: "refresh",
        ttl: {7, :days})
