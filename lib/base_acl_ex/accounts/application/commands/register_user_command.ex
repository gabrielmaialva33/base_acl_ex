defmodule BaseAclEx.Accounts.Application.Commands.RegisterUserCommand do
  ...
  def validate(command) do
    ...
    errors = if is_nil(command.password) || String.length(command.password) < 8 do
      [{:password, "must be at least 8 characters"} | errors]
    else
      errors
    end
    
    errors = if is_nil(command.first_name) || command.first_name == "" do
      [{:first_name, "is required"} | errors]
    else
      errors
    end
    
    errors = if is_nil(command.last_name) || command.last_name == "" do
      [{:last_name, "is required"} | errors]
    else
      errors
    end
    
    errors = if command.terms_accepted != true do
  ...
  def metadata(command) do
    %{
      ...
      ip_address: nil # To be set by the handler
    }
  end
