defmodule BaseAclEx.Identity.Core.Entities.Role do
  ...
  @optional_fields [:description, :parent_role_id, :is_active, :is_system,
                    :color, :icon, :max_users, :features, :metadata]
  ...
  def update(%__MODULE__{} = role, attrs) do
    role
    |> cast(attrs, [:name, :description, :is_active, :color, :icon, 
                   :max_users, :features, :metadata])
    |> validate_name()
    |> validate_max_users()
  end
  ...
  defp validate_slug(changeset) do
    validate_format(changeset, :slug, ~r/^[a-z][a-z0-9_]*$/, 
      message: "must start with letter and contain only lowercase letters, numbers and underscores")
  end
