defmodule BaseAclEx.Identity.Core.Entities.Permission do
  ...
  @optional_fields [:name, :context, :description, :category, :is_active, :is_system,
                    :requires_ownership, :requires_two_factor, :risk_level, 
                    :conditions, :dependencies, :metadata]
  ...
  def update(%__MODULE__{} = permission, attrs) do
    permission
    |> cast(attrs, [:description, :category, :is_active, :conditions, 
                   :dependencies, :metadata, :requires_two_factor])
    |> validate_conditions()
  end
  ...
  defp validate_context(changeset) do
    validate_inclusion(changeset, :context, 
  ...
  defp validate_risk_level(changeset) do
    validate_inclusion(changeset, :risk_level, 
